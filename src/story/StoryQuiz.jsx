...
export default function StoryQuiz({
  ...
  const [activeQuizItems, setActiveQuizItems] = useState([]);
  ...
  const activeTotal = activeQuizItems.length;
  const scoreRatio = activeTotal > 0 ? score / activeTotal : 0;
  const scorePercent = Math.round(scoreRatio * 100);
  const isPerfectScore = finished && activeTotal > 0 && score === activeTotal;
  ...
  useEffect(() => {
    const current = activeQuizItems[index];
    if (!current) return;
  ...
  const currentQuestion = activeQuizItems[index];
  if (!currentQuestion) {
    return (
      <div className="story-quiz">
  ...
  const handleAnswer = (option) => {
    ...
    window.setTimeout(() => {
      setFeedback(null);
      if (index + 1 < activeQuizItems.length) {
  ...
  const startRetryRound = () => {
    ...
    setActiveQuizItems(shuffleArray(missedQuestions));
    setIndex(0);
    setScore(0);
    setOptions([]);
    setSelectedAnswer(null);
  ...
  if (finished) {
    ...
    if (isRetryRound) {
      resultHeadline = "Retry round complete - Uusintakierros valmis";
      if (isPerfectScore) {
        resultSubtext = "Great recovery! - Hieno korjaus!";
      }
    } else {
      if (scoreRatio >= 0.8) {
    ...
    return (
      <div className="story-quiz story-quiz-end quiz-done">
        ...
        <p>
          Score: {score} / {activeTotal} ({scorePercent}%)
        </p>
        {!isRetryRound && missedQuestions.length > 0 && (
          <button className="ghost-btn story-action-btn" type="button" onClick={startRetryRound}>
            Try missed words again
          </button>
        )}
